...
--              b1bf3cc9-cece-407a-ba3d-b6a76bcb0840     → UUID of the guard auth user you just created
--              8421089343         → phone number the guard will type in the app (e.g. +919876543210)
--              e95ab2a2-06a8-47f4-a127-bc118daebcd3  → UUID of the resident auth user
--              8421089343      → phone number the resident will type in the app
--
...
-- ============================================================
-- PART 2: Guard setup
-- Replace b1bf3cc9-cece-407a-ba3d-b6a76bcb0840 and 8421089343 before running.
-- ============================================================

DO $$
DECLARE
  v_guard_auth_id  UUID := 'b1bf3cc9-cece-407a-ba3d-b6a76bcb0840';        -- ← REPLACE
  v_guard_phone    TEXT := '+918421089343';          -- ← REPLACE (e.g. +919876543210)
  ...
  v_role_id        UUID;
BEGIN
  IF v_guard_auth_id::TEXT = 'b1bf3cc9-cece-407a-ba3d-b6a76bcb0840' THEN
    RAISE EXCEPTION 'Replace b1bf3cc9-cece-407a-ba3d-b6a76bcb0840 with the real auth user UUID from Studio.';
  END IF;

  IF v_guard_phone = '+918421089343' THEN
    RAISE EXCEPTION 'Replace 8421089343 with the guard''s test phone number.';
  END IF;
...
-- Replace e95ab2a2-06a8-47f4-a127-bc118daebcd3 and 8421089343 before running.
-- ============================================================

DO $$
DECLARE
  v_resident_auth_id UUID := 'e95ab2a2-06a8-47f4-a127-bc118daebcd3';   -- ← REPLACE
  v_resident_phone   TEXT := '+918421089343';    -- ← REPLACE (e.g. +919999988888)
  v_resident_email   TEXT := 'resident.demo@facilitypro.local';
  v_resident_id      UUID := '20000000-0000-0000-0000-000000000020';
  v_role_id          UUID;
BEGIN
  IF v_resident_auth_id::TEXT = 'e95ab2a2-06a8-47f4-a127-bc118daebcd3' THEN
    RAISE EXCEPTION 'Replace e95ab2a2-06a8-47f4-a127-bc118daebcd3 with the real auth user UUID from Studio.';
  END IF;

  IF v_resident_phone = '+918421089343' THEN
    RAISE EXCEPTION 'Replace 8421089343 with the resident''s test phone number.';
  END IF;
...
--   DEMO_OTP_ALLOWED_PHONES  = +918421089343,+918421089343
--
-- Also update Solvesxx_web/.env.local (already done):
--   Replace +918421089343 and +918421089343 with the actual numbers.
-- ============================================================
