...
FLOW_PATH="$REPO_ROOT/qa_agent/maestro/guard_resident_staging_deny_e2e.yaml"
...
TARGET_VISITOR_NAME="Staging Deny Visitor 01"
APPROVE_VISITOR_NAME="Staging Approve Visitor 01"
...
echo "Force-stopping the app for a clean guard/resident deny staging run"
...
where flat_id = '$TARGET_FLAT_ID'
  and visitor_name in ('$TARGET_VISITOR_NAME', '$APPROVE_VISITOR_NAME');
" --linked >/dev/null

echo "Running guard resident deny staging flow: $FLOW_PATH"
QA_MAESTRO_MAX_ATTEMPTS="${QA_MAESTRO_MAX_ATTEMPTS:-1}" \
  "$REPO_ROOT/qa_agent/run_maestro_smoke.sh" "$FLOW_PATH"

echo "Verifying denied visitor in Supabase"
...
if [[ "$VISITOR_RESULT" != *"$TARGET_VISITOR_NAME"* ]]; then
  echo "No deny staging visitor row was created." >&2
  exit 1
fi

if [[ "$VISITOR_RESULT" != *"denied"* ]]; then
  echo "Visitor row was created but did not reach denied state." >&2
  exit 1
fi
